...
unsafe extern "C" fn mock_hsm_generate_mac(
    ...
    if out_mac.is_null() || out_mac_len.is_null() {
        return -2;
    }
    for i in 0..16_usize {
        unsafe {
...
fn crypto_csm_encrypt_matches_direct_hsm() {
    ...
    assert_eq!(
        &ciphertext[..],
        &expected[..],
        "Csm_Encrypt output should match direct hsm_encrypt (XOR 0xAA)"
    );
...
fn crypto_csm_mac_generate_and_verify() {
    ...
    assert!(
        mac.iter().all(|&b| b == 0xBB),
        "Mock MAC should be all 0xBB"
    );
...
fn crypto_csm_random_generate() {
    ...
    assert!(
        buf.iter().any(|&b| b != 0),
        "Random bytes should be non-zero"
    );
    assert_eq!(buf[0], 1);
    assert_eq!(buf[31], 32);
...
fn crypto_security_access_flow() {
    ...
    let rc = unsafe { (csm_key_validate)(key.as_ptr(), key_len) };
